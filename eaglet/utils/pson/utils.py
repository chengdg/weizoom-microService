...
def get_path_value(data, stringify):
	...
	def _recursive_get(data, res, path='/'):
		...
		if isinstance(data, dict):
			...
			if len(data) == 0:
				res.append(_get_format_str(path, []))
			...
			res.append(_get_format_str("/ERROR", str(type(data))))
...
def parse(pson_string):
	pieces = pson_string.split(',,')
	patch = []
...
def add(data, jpath, value, replace=False):
	...
	match, remainder, d, reason = find(data, jpath)

	if reason == 'type':
		raise TypeError('Bad subdoc type after {}'.format(join(match)))

	if not reason:
		if replace:
			d = resolve(data, match[:-1])
			d[match[-1][1]] = value
	else:
		sub_doc = create(remainder[1:], value)
		name = remainder[0][1]
		if reason == 'key':
			d[name] = sub_doc
		elif reason == 'index':
